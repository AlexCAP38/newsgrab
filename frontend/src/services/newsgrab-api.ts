...
export interface News {
  ...
  /** Дата публикации */
  date: string;
  /** Id пользователя принадлежащая новость */
  userId: string;
  /** Источник новости */
  resource: string;
}
...
export class HttpClient<SecurityDataType = unknown> {
  ...
  constructor({ securityWorker, secure, format, ...axiosConfig }: ApiConfig<SecurityDataType> = {}) {
    this.instance = axios.create({ ...axiosConfig, baseURL: axiosConfig.baseURL || "http://localhost:8888" });
    this.secure = secure;
    this.format = format;
...
export class Api<SecurityDataType extends unknown> extends HttpClient<SecurityDataType> {
  ...
  user = {
     ...
     */
    registrationCreate: (data: UserRegistration, params: RequestParams = {}) =>
      this.request<UserSuccessfully, any>({
